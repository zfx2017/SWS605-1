...
;------------------------------------------------------------
;x                         Allocated with name '_mInitSTDIO_x_1_16'
;x2                        Allocated with name '_mInitSTDIO_x2_1_16'
;------------------------------------------------------------
...
;	../include/debug.c:173: x = 10 * FREQ_SYS / UART0_BAUD / 16;                                       //如果更改主频，注意x的值不要溢出
	mov	_mInitSTDIO_x_1_16,#0x11
	mov	(_mInitSTDIO_x_1_16 + 1),#0x04
	clr	a
	mov	(_mInitSTDIO_x_1_16 + 2),a
	mov	(_mInitSTDIO_x_1_16 + 3),a
;	../include/debug.c:174: x2 = x % 10;
	...
	mov	dpl,_mInitSTDIO_x_1_16
	mov	dph,(_mInitSTDIO_x_1_16 + 1)
	mov	b,(_mInitSTDIO_x_1_16 + 2)
	mov	a,(_mInitSTDIO_x_1_16 + 3)
	lcall	__modulong
	mov	r4,dpl
	mov	_mInitSTDIO_x2_1_16,r4
;	../include/debug.c:175: x /= 10;
	...
	mov	a,(_mInitSTDIO_x_1_16 + 3)
	lcall	__divulong
	mov	_mInitSTDIO_x_1_16,dpl
	mov	(_mInitSTDIO_x_1_16 + 1),dph
	mov	(_mInitSTDIO_x_1_16 + 2),b
	mov	(_mInitSTDIO_x_1_16 + 3),a
;	../include/debug.c:176: if ( x2 >= 5 ) x ++;                                                       //四舍五入
	mov	a,#0x100 - 0x05
	add	a,_mInitSTDIO_x2_1_16
	jnc	00102$
	mov	a,#0x01
	add	a,_mInitSTDIO_x_1_16
	mov	_mInitSTDIO_x_1_16,a
	clr	a
	addc	a,(_mInitSTDIO_x_1_16 + 1)
	mov	(_mInitSTDIO_x_1_16 + 1),a
	clr	a
	addc	a,(_mInitSTDIO_x_1_16 + 2)
	mov	(_mInitSTDIO_x_1_16 + 2),a
	clr	a
	addc	a,(_mInitSTDIO_x_1_16 + 3)
	mov	(_mInitSTDIO_x_1_16 + 3),a
00102$:
...
;------------------------------------------------------------
;	../include/debug.c:215: void putchar(char c)
;	-----------------------------------------
;	 function putchar
;	-----------------------------------------
_putchar:
	mov	r7,dpl
;	../include/debug.c:217: while (!TI) /* assumes UART is initialized */
00101$:
;	../include/debug.c:219: TI = 0;
	jbc	_TI,00112$
	sjmp	00101$
00112$:
;	../include/debug.c:220: SBUF = c;
	mov	_SBUF,r7
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'getchar'
;------------------------------------------------------------
;	../include/debug.c:223: char getchar() {
;	-----------------------------------------
;	 function getchar
;	-----------------------------------------
_getchar:
;	../include/debug.c:224: while(!RI); /* assumes UART is initialized */
00101$:
;	../include/debug.c:225: RI = 0;
	jbc	_RI,00112$
	sjmp	00101$
00112$:
;	../include/debug.c:226: return SBUF;
	mov	dpl,_SBUF
	ret
;------------------------------------------------------------
